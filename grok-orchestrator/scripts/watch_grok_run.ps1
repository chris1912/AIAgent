# Watches one Grok run without repeatedly sending logs through model context.
# Artifact-progress stall detection; optional aggregate multi-worker observation.
# Authored 2026-07-12; upgraded 2026-07-18.
[CmdletBinding()]
param(
    ...
    # When set, monitor every workers/*/WORKER_PID.txt (and WORKER_PIDS.json).
...
function Get-TargetProcess {
    ...
    return (Get-GoTargetProcess -Id $Id -ExpectedStartUtc $ExpectedStartUtc)
}

function Stop-TargetProcessTree {
    param([int]$RootId, [datetime]$ExpectedStartUtc)
    Stop-GoProcessTree -RootId $RootId -ExpectedStartUtc $ExpectedStartUtc
    return
}
...
function Get-WorkerTargets {
    ...
    if ($Aggregate) {
        ...
        $workersRoot = Join-Path $RunDir 'workers'
        if (Test-Path -LiteralPath $workersRoot) {
            $workerDirs = @(Get-ChildItem -LiteralPath $workersRoot -Directory -ErrorAction SilentlyContinue)
    ...
    if (-not $pidPath) { throw "Missing Grok PID file in $RunDir" }
    $procId = [int](Get-Content -LiteralPath $pidPath -Raw).Trim()
    $proc = Get-Process -Id $procId -ErrorAction SilentlyContinue
    if ($null -eq $proc) { throw "Process $procId from $pidPath is not running" }
    $targets += [pscustomobject]@{
        worker_id = 'root'
        process_id = $procId
